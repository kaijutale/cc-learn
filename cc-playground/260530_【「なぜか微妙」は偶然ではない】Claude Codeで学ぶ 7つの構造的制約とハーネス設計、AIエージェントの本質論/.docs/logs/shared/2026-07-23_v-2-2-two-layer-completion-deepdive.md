...
- **[Low→issue #221 前倒し] warn 型の恒久バイパス + 凝集度判断の非観測性** (かいじゅうレビュー 2026-07-23 で観点追加): 「行数は合図」の設計宣言を認める限り、「凝集度的に OK」の合理化で**今後の 500 行超も毎回通過できる** (機械の閉塞点なし = V-2 の開いたループの一種)。かつ「凝集度で判断して残した」か「読み飛ばした」かを**事後に区別できない** (V-2.1 観測可能性違反、soft-K と同型)。block 化は Goodhart (500行を満たすための機械分割 = 凝集度悪化) を誘発するため解でなく、対策候補は (a) ラチェット = 新規超過のみ強い warn (before/after 比較は決定論) / (b) 判断痕跡化 = warn 発火時に凝集度判断を ledger 記録 / (c) 再発計数 = check-gotcha-recurrence パターン流用 (#69 の hook-fire ledger に発火記録が既にあるか要確認)。実害未発生だが**かいじゅうの明示判断で再発待ち規律を前倒し**し issue #221 (kaijutale/claude-harness) として計画作業化
...
判定: 取り入れ済み — note の具体例 3 種 (500行/循環依存/配置) すべてに実装が実在し、二層 AND は「検証の二層構成（V-2.2）」の名で正典化、「片方のみ pass は不完了」と担い手分離 (tester/reviewer) まで揃う。記事超えは強制度の勾配・structure 概念のメタ層拡張・madge 非依存 DFS・差し戻し分岐の設計織込み。残差は「structure 側の機械 gate が behavior 側より弱い」非対称 Low×4 + 観測 1 で、いずれも自認済みロードマップ or 意図的 warn 設計。**これで V章の単独深掘りキュー 6 ユニット (V-1.1 / V-1.2 / V-1.3 / V-2 / V-2.1 / V-2.2) が完走**。V-1 (決定論ファースト) 自体は 07-20 親バッチの 1 行判定のみで単独深掘り未実施 (深掘りキューに元々積まれていない — 初稿は「全 7 ユニットの深掘り完了」と過大表現しており、handoff 書出し時の probe 検証 (反例狩り: ログ実在の実測) で検出し訂正)。
