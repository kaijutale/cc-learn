...
|---|---------|----------------|-------------------|
| **本質** | git worktreeの**操作ツール** | AIエージェントの**開発ワークフロー** | Claude Codeの**実践的Tips集**（skill化済み） |
| **レイヤー** | インフラ（道具） | プロセス（仕組み） | ナレッジ（知恵） |
| **例えるなら** | 高性能な包丁 | レシピブック | シェフの経験則 |
| **作者** | CodeRabbit社 | Jesse Vincent (obra) | Boris Cherny (Claude Code開発者) |
| **形態** | CLIツール（Bash製） | Claude Codeプラグイン | Claude Code **skill** + ブログ記事 |
| **インストール** | `brew install git-gtr` | `/plugin install superpowers` | ワンライナーでskillインストール |
| **呼び出し** | `git gtr <command>` | `/superpowers:<skill>` | `/boris` |
...
Claude Code開発者であるBoris Cherny氏が公開した実践的なTips集。全35選（Part 1: 13選、Part 2: 10選、Part 3: 12選）。
**Claude Codeのskillとしてインストール可能**で、`/boris`で呼び出せる。
...
| Part | 時期 | Tips数 | テーマ |
|------|------|--------|--------|
| Part 1 | 2025年12月 | 13選 | 基本的な活用術（並列開発、Plan Mode、検証） |
| Part 2 | 2026年1月 | 10選 | 応用テクニック（worktree、Skills、プロンプティング） |
| Part 3 | 2026年2月 | 12選 | カスタマイズ（hooks、plugins、agents、sandbox） |
...
- ターミナル設定（テーマ、通知、vim mode、`/terminal-setup`）
- エフォートレベル調整（`/model`でLow/Medium/High）
- プラグイン・MCP・Skillsのインストール（`/plugin`）
- カスタムエージェント（`.claude/agents/`にmd配置、デフォルトエージェント設定可能）
- サンドボックス（`/sandbox`でファイル・ネットワーク分離）
- ステータスライン（`/statusline`でコンテキスト使用量・gitブランチ表示）
- キーバインド（`/keybindings`で全キー設定カスタマイズ可能）
- フック（ライフサイクルにフックして権限ルーティング、ログ、自動継続等）
- スピナー動詞のカスタマイズ（settings.jsonで独自動詞設定）
- 出力スタイル（`/config`でExplanatory、Learning、Custom等）
- 37の設定項目 + 84の環境変数で細かくカスタマイズ可能
